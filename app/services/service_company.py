...
from app.utils.transform import safe_convert_float, safe_get_float, safe_get_int, safe_get_str
...
def get_company_overview(ticker: str) -> CompanyOverviewTCBS:
    ...
    try:
        ...
        raise Exception(f"Failed to retrieve company overview for {ticker}: {str(e)}") from e
...
def get_company_profile(ticker: str) -> CompanyProfile:
    ...
    try:
        ...
        raise Exception(f"Failed to retrieve company profile for {ticker}: {str(e)}") from e
...
def get_company_shareholders(ticker: str) -> list[CompanyShareholders]:
    ...
    try:
        ...
        for _, row in df.iterrows():
            shareholders.append(CompanyShareholders(
        ...
        raise Exception(f"Failed to retrieve company shareholders for {ticker}: {str(e)}") from e
    ...
    try:
        ...
        for _, row in df.iterrows():
            officers.append(CompanyOfficer(
                ...
                officer_own_percent=safe_get_float(row, 'officer_own_percent') or 0.0,
            ))

        return officers

    except Exception as e:
        raise Exception(f"Failed to retrieve company officers for {ticker}: {str(e)}") from e
...
def get_company_subsidiaries(ticker: str) -> list[CompanySubsidiaries]:
    ...
    try:
        ...
        subsidiaries = []
        for _, row in df.iterrows():
            subsidiaries.append(CompanySubsidiaries(
        ...
        raise Exception(f"Failed to retrieve company subsidiaries for {ticker}: {str(e)}") from e
    ...
    try:
        ...
        for _, row in df.iterrows():
            dividends.append(DividendHistory(
                ...
                cash_dividend_percentage=safe_get_float(row, 'cash_dividend_percentage') or 0.0,
        ...
        raise Exception(f"Failed to retrieve company dividends for {ticker}: {str(e)}") from e
...
def get_company_insider_deals(ticker: str) -> list[CompanyInsiderDeals]:
    ...
    try:
        ...
        insider_deals = []
        for _, row in df.iterrows():
            insider_deals.append(CompanyInsiderDeals(
        ...
        raise Exception(f"Failed to retrieve company insider deals for {ticker}: {str(e)}") from e
...
def get_company_events(ticker: str) -> list[CompanyEventsTCBS]:
    ...
    try:
        ...
        events = []
        for _, row in df.iterrows():
            events.append(CompanyEventsTCBS(
                ...
                price_change_ratio_1m=safe_get_float(row, 'price_change_ratio_1m') or 0.0,
        ...
        raise Exception(f"Failed to retrieve company events for {ticker}: {str(e)}") from e
    ...
    try:
        ...
        for _, row in df.iterrows():
            news_list.append(CompanyNews(
                ...
                price_change_ratio=safe_get_float(row, 'price_change_ratio') or None,
                price_change_ratio_1m=safe_get_float(row, 'price_change_ratio_1m') or None,
                id=safe_get_int(row, 'id') or 0,
        ...
        raise Exception(f"Failed to retrieve company news for {ticker}: {str(e)}") from e
    ...
    try:
        ...
        for _, row in df.iterrows():
            ratios.append(CompanyRatioVCI(
                ...
                charter_capital_ratio=safe_get_float(row, 'charter_capital_ratio') or 0.0,
        ...
        raise Exception(f"Failed to retrieve company ratios for {ticker}: {str(e)}") from e
...
def get_company_reports_vci(ticker: str) -> list[CompanyReportsVCI]:
    ...
    try:
        ...
        reports = []
        for _, row in df.iterrows():
            reports.append(CompanyReportsVCI(
        ...
        raise Exception(f"Failed to retrieve company reports for {ticker}: {str(e)}") from e
...
def get_company_trading_stats_vci(ticker: str) -> TradingStatsVCI:
    ...
    try:
        ...
        raise Exception(f"Failed to retrieve trading statistics for {ticker}: {str(e)}") from e