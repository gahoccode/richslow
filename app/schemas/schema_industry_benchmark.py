...
class IndustryBenchmark(BaseModel):
    ...
    benchmarks: dict[str, RatioBenchmark] = Field(..., description="Ratio benchmarks by name")
    ratios_available: list[str] = Field(..., description="List of available ratio benchmarks")


class IndustryBenchmarkRequest(BaseModel):
    """Request model for industry benchmark data."""
    industry_id: int | None = Field(None, description="Industry ID (ICB classification)")
    industry_name: str | None = Field(None, description="Industry name for lookup")
    min_companies: int = Field(5, description="Minimum companies required for valid benchmark")