...
class TestCompanyAPIIntegration:
    ...
    def test_complete_company_overview_flow(self, mock_company_class, client):
        ...
        assert "@" in data["website"] if data["website"] else True  # Valid email format if present
    ...
    @patch('app.services.service_company.TCBSCompany')
    @patch('app.services.service_company.VCICompany')
    def test_multiple_data_sources_consistency(self, mock_vci_class, mock_tcbs_class, client):
        ...
        assert len(overview_data["short_name"]) > 0 if overview_data.get("short_name") else True
...
class TestCompanyAPIContract:
    ...
    def test_response_format_compliance(self, mock_company_class, client):
        ...
        required_fields = [
            "exchange", "industry", "company_type", "no_shareholders",
            "foreign_percent", "outstanding_share", "established_year",
            "short_name"
        ]
