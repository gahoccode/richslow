...
class TestServiceLayerIsolation:
    ...
    def test_get_financial_statements_mocked(
        self,
        mock_vnstock_class,
        ...
        request = StatementsRequest(
            ticker="TEST", start_date="2023-01-01", end_date="2023-12-31", period="year"
        )
...
class TestConfigurationFlexibility:
    ...
    def test_different_data_sources(self, mock_vnstock_class):
        ...
        mock_stock_instance.finance.balance_sheet.return_value = pd.DataFrame(
            {"yearReport": [2023]}
        )
        mock_stock_instance.finance.income_statement.return_value = pd.DataFrame(
            {"yearReport": [2023]}
        )
        mock_stock_instance.finance.ratio.return_value = pd.DataFrame(
            {"yearReport": [2023]}
        )
...
class TestExternalApplicationIntegration:
    ...
    def test_microservice_style_integration(self):
        ...
        input_data = [{"yearReport": 2023, "P/E": 15.5, "ROE (%)": 0.225}]
