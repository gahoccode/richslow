...
class TestAPIIntegration:
    ...
    def test_get_financial_statements_endpoint_success(
        self,
        mock_vnstock_class,
        client,
    ...
    def test_api_response_schema_validation(self, client):
        ...
        with patch("app.services.service_statements.Vnstock") as mock_vnstock_class:
            ...
            mock_finance_instance.cash_flow.return_value = pd.DataFrame(
                {"yearReport": [2023]}
            )
            mock_finance_instance.balance_sheet.return_value = pd.DataFrame(
                {"yearReport": [2023]}
            )
            mock_finance_instance.income_statement.return_value = pd.DataFrame(
                {"yearReport": [2023]}
            )
            mock_finance_instance.ratio.return_value = pd.DataFrame(
                {"yearReport": [2023]}
            )
...
class TestFrontendBackendIntegration:
    ...
    @patch("app.services.service_statements.Vnstock")
    def test_frontend_field_contract_fulfillment(
        self, mock_vnstock_class, client, sample_vnstock_ratios_data
    ):
        ...
        if data["ratios"]:
            ...
            for field in frontend_critical_fields:
                assert field in ratio_data, (
                    f"Critical frontend field {field} missing from API response"
                )
...
class TestErrorHandlingAndResilience:
    ...
    def test_partial_data_handling(self, mock_vnstock_class, client):
        ...
        mock_finance_instance.cash_flow.side_effect = Exception(
            "Cash flow data unavailable"
        )
        mock_finance_instance.balance_sheet.side_effect = Exception(
            "Balance sheet unavailable"
        )
        mock_finance_instance.income_statement.side_effect = Exception(
